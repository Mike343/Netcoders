...
namespace Coders.Tests.Services
{
	...
	public class AuditServiceTest
	{
		...
		public void Test_AuditService_Audit()
		{
			...
			Assert.AreEqual(1, audit.Items[0].ParentId, "Items.ParentId");
			Assert.AreEqual(test.GetType().ToString(), audit.Items[0].Type, "Items.Type");
			Assert.AreEqual(AuditAction.Create, audit.Items[0].Action, "Items.Action");
			Assert.AreEqual("Test", audit.Items[0].User.Name, "Items.User.Name");

			var value = audit.Items[0].GetEntity<AuditTest>();

			Assert.AreEqual("test", value.Value, "Value");
			Assert.AreEqual(10, value.Value2, "Value2");
			Assert.AreEqual(test.Value3, value.Value3, "Value3");
		}
